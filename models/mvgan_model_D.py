...
import torch
weight_D = ['weighted_multiscale', 'image']
...
class mvganD(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netD_img = define_D(opt, opt.d_type)
        ...
        if opt.net_type in vid_net:
            self.netD_vid = define_D(opt, 'multiscale_vid')
        elif opt.net_type in img_net:
            self.tD = 1
        ...
        self.optimizer_D_T = torch.optim.Adam(self.netD_vid.parameters(), lr=self.old_lr, betas=(beta1, beta2)) if opt.net_type in vid_net else None
        ...
        self.criterionStruct = StructLoss(opt) if not self.opt.no_struct else None
        self.criterionTexture = TextureLoss() if not self.opt.no_texture else None
    ...
    def forward(self, tensor_list, type='video', dummy_bs=0):
        ...
        if type == 'video':
            if self.net_type in vid_net and self.scale == 0:
        ...
        loss_G_Texture = (self.criterionTexture(fake_B, real_B) * lambda_texture) if not self.opt.no_texture else self.Tensor(1,1).fill_(0.0)
    ...
    def get_losses(self, loss_dict, loss_dict_T):
        ...
        if self.opt.net_type in vid_net and loss_dict_T is not None:
    ...
    def GAN_and_FM_loss(self, pred_real, pred_fake):
        ...
        if not self.opt.no_ganFeat:
            feat_weights = 4.0 / (self.opt.d_n_layers + 1)
    ...
    def compute_loss_D_T(self, netD_T, real_B, fake_B, real_A):
        ...
        if self.d_type in weight_D:
            ...
            pred_real = netD_T(concat_real)
            pred_fake = netD_T(concat_fake.detach())
