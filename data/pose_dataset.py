...
class PoseDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        A, B, seq_idx = self.update_frame_idx(self.img_paths, index)
        img_paths = self.img_paths[seq_idx]
        n_frames_total, start_idx, t_step = get_video_params(self.opt, self.n_frames_total, len(img_paths),
                                                             self.frame_idx)
    ...
    def get_image(self, A_path, size, params, input_type):
        if input_type != 'openpose':
            ...
            A_img = Image.fromarray(
                read_keypoints(A_path, size, random_drop_prob, self.opt.remove_face_labels, self.opt.basic_point_only))
    ...
    def normalize_pose(self, A_img, target_yc, target_len, first=False):
        ...
        A_np = A_np[self.offset_y:int(self.offset_y + h / self.ratio),
               self.offset_x:int(self.offset_x + w / self.ratio):, :]
        A_img = Image.fromarray(A_np)
